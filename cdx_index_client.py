...
def fetch_result_page(job_params):
    ...
    if os.path.exists(filename):
        # this has already been processed: skip
        logging.debug('File {} already exists, skipping ...'.format(filename))
        return
...
def do_work(job_queue, counter=None):
    ...
    while not job_queue.empty():
        try:
            ...
            wait_time = job.get('retries', 0) * job['retry_wait']
            if wait_time:
                logging.info(
                    "waiting {}s to retry {} page {} ...".format(wait_time, job['output_prefix'], job['page'])
                )
                sleep(wait_time)
            ...
            if retries < job['max_retries']:
                logging.error('Retrying {} Page {}'.format(job['api_url'], job['page']))
                job['retries'] = retries + 1
                job_queue.put_nowait(job)
            else:
                logging.error('Max retries exceeded for page {0}'.
                              format(job['page']))
                filename = "{}-{}".format(job['output_prefix'], job['page'])
                # use dir, if provided
                if job['dir']:
                    if not os.path.isdir(job['dir']):
                        os.makedirs(job['dir'])
                    filename = os.path.join(job['dir'], filename)
                with open('failed.txt', 'a') as ofile:
...
def get_args():
    ...
    *.example.com
    ...
    parser.add_argument('--retry-wait', default=0, type=int,
                        help='Wait seconds before retry, per attempt (multiplicative)')
...
def read_index(r, api_url, prefix=None):
    ...
    # set num workers based on processes
    if not r.processes:
        try:
